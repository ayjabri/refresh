...
if __name__=='__main__':
    ...
    parser.add_argument('--cuda', action='store_true', help='Activate GPU in training')
    parser.add_argument('--steps', default=1, type=int, help='Number of training steps to skip')
    ...
    envs = utils.createEnvs(params.env,n=4,seed=params.seed)
    ...
    exp_src = ptan.experience.ExperienceSourceFirstLast(envs, agent, params.gamma, steps_count=args.steps)
    ...
    print(net)
    print('*'*10, ' Start Training ',envs[0].game, ' {} '.format(device), '*'*10)
            ...
            if reward:
                ...
                if mean:
                    if int(mean) > best_reward:
                        ...
                        save_path = os.path.join(folder,sub_folder,str(int(best_reward)) +'_' + save_time + '_.dat')

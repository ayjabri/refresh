...
if __name__=='__main__':
    ...
    parser.add_argument('-c', '--cuda', action='store_true', help='Train on GPU')
    parser.add_argument('-p', '--priority', action='store_true', help='Use Priority Replay Buffer')
    parser.add_argument('-s','--steps', default=4, type=int, help='steps to skip while training')
    parser.add_argument('-n','--envs', default= 3, type=int, help='Number of environments to run simultaneously')
    parser.add_argument('-g', '--game', default='invaders', help='OpenAI gym environment name')
    ...
    eps_tracker = ptan.actions.EpsilonTracker(selector, params.eps_start, params.eps_final, params.eps_frames*args.envs)
    ...
    comment = "".join([envs[0].game,'_',str(args.steps),'_',str(args.envs)])
    ...
    print(net)
    print(5*'*',f' Training {envs[0].game}: {device}/{args.envs} environments/{args.steps} steps', '*'*5)
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        for frame, exp in enumerate(exp_source):
            ...
            buffer.add(state=exp.state, action=exp.action, reward=exp.reward, last_state=last_state, done=done)
            eps_tracker.frame(frame)
            reward = exp_source.pop_total_rewards()
            if reward:
                mean = tracker.reward(reward[0], frame, epsilon=selector.epsilon)
                
                if mean:
                    if int(mean) > best_reward:
                        best_reward = int(mean)
                        torch.save(net.state_dict(),f= os.path.join(folder,sub_folder, str(best_reward) +'.dat'))
                    if mean > params.solve_rewards:
                        print('Solved in {}!'.format(datetime.now()-st))
                        if args.play: utils.play(params.env, agent, wait=0.01)
                        break
            ...
            if frame%args.envs==0:
                ...
                if args.priority:
                    ...
                    loss_v= calc_loss_dqn(batch, net, tgt_net, params.gamma**args.steps,device=device)
                loss_v.backward()
                optimizer.step()
                if args.priority:
